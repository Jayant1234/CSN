...
class Retrain_Autodeeplab(nn.Module):
    def __init__(self, args):
        ...
        if (not args.dist and args.use_ABN) or (args.dist and args.use_ABN and dist.get_rank() == 0):
            print("=> use ABN!")
        if args.net_arch is not None and args.cell_arch is not None and args.network_path is not None:
            network_archie, cell_arch, network_path = np.load(args.net_arch), np.load(args.cell_arch),np.load(args.network_path)
            network_arch= network_layer_to_space(network_path)
        else:
            network_arch, cell_arch, network_path = get_default_arch()
        #print("Cell Architecture=", cell_arch)
        #print('Network Path=', network_path)
        #print('Network Architecture:',network_arch)
        ...
        self.decoder = Decoder(args.num_classes, filter_multiplier=args.filter_multiplier * args.block_multiplier, #multiplied two for this case only!
