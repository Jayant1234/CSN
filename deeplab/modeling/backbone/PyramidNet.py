...
class PyramidNet(nn.Module):
    ...
    def __init__(self, dataset, depth, alpha, num_classes, bottleneck=False,pretrain=True):
        ...
        for m in self.modules(): # what is this doing? Should pretrained model come after this? 
        ...
        if(pretrain and dataset=="imagenet"): 
            self.load_state_dict(torch.load("./PyramidNet_model/pyrmaidnet101_224x224_pretrained.pth"))
            # loadstate dict of everything but the last part from image and put num_class=pa here only because the model does not adapt the layer to batch size after certain point WHy? 
    ...
    def forward(self, x):
        if self.dataset == 'cifar10' or self.dataset == 'cifar100':
            ...
            x = self.relu_final(x)
            x = self.avgpool(x)
            #print("Size before view")
            #print(x.size())
            x = x.view(x.size(0), -1)
            #print("Size after view")
            #print(x.size())
            x = self.fc(x)
